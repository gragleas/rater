...
df = pd.read_csv('/home/graham/Downloads/artist-uris.csv')
...
class spotipyAgent:
	...
	def getClosestArtists(self, test_artist):
		best_matches = []
		similarities = []

		for i in range(len(col_list)):#col_list:
			if len(best_matches) < 3:
				best_matches.append(col_list[i])
				similarities.append(self.similar(test_artist, str(col_list[i]).lower()))
			else:
				min_index = similarities.index(min(similarities))
				if self.similar(test_artist, str(col_list[i]).lower()) > similarities[min_index]:
					best_matches[min_index] = col_list[i]
					similarities[min_index] = self.similar(test_artist, str(col_list[i]).lower())

		return(best_matches)
		
	def searchTrack(self, name, artists):
		best_match = ("", "", "", 0)
		for artist in artists:
			results = self.sp.search(q='artist:' + artist, type='album', limit=10, offset=0)
			...
			for album in items:
				...
				for track in tracks:
					...
					score = self.similar(name, tra) + self.similar(artist, art.lower())
					if score > best_match[3]:
						best_match = (track["uri"], tra, art, score)
		#print(best_match)
		if best_match[3] >= 1.5:
			self.sp.add_to_queue(uri=best_match[0])
			self.skip()
			return 1
		...
		try:
			...
			if split[0] == "T" or split[0] == "t":
				try:
					...
					artists = agent.getClosestArtists(artist_name)
					agent.searchTrack(track_name, artists)
				except:
